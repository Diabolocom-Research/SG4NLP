...
class OpenRouterLLM:
    def __init__(self, model_name, temperature):
        ...
        if not self.api_key:
            logger.warning("OPENROUTER_KEY not set in environment variables.")
        self.client = OpenAI(
...
class OpenRouterLLMAdapter(BaseLLM):
    """
    ...
    def __init__(
        ...
        self.temperature = temperature
        self.llm_ep = OpenRouterLLM(model_name=self.model_name, temperature=self.temperature)
        # Wrap the LLM with caching using functools.partial.
        self.llm = partial(
            caching_layer_openrouter,
...
if __name__ == '__main__':
    try:
        ...
        openai_llm = OpenRouterLLM(model_name="gpt-4o-mini", temperature=0.0)
        # prompt = (
